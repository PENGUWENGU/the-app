...
struct MapHomeView: View {
    ...
    var body: some View {
        ...
        .onReceive(NotificationCenter.default.publisher(for: .locusSaveCurrentRouteRequest)) { note in
            guard let name = note.object as? String else { return }
            if let track = importedGPXTrack, !track.isEmpty {
                session.saveRoute(name: name, track: track)
            } else {
                let path = routeCoords.isEmpty ? drawnPath : routeCoords
                if path.count >= 2 {
                    session.saveRoute(name: name, coordinates: path)
                } else {
                    session.lastError = "Create a route with at least 2 points before saving."
                }
            }
        }
        .fileImporter(
        ...
        .sheet(isPresented: $showRouteSheet) {
            RoutePlannerSheet(
                ...
                onSelectSavedRoute: { saved in
                    importedGPXTrack = nil
                    routeCoords = saved.clCoordinates
                    drawnPath.removeAll()
                    drawMode = false
                    if let first = saved.clCoordinates.first {
                        session.pin = first
