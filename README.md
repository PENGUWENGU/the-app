...
**iOS 26+ (including iOS 26.6.1):** Settings → **Pair on this iPhone** advertises `_remotepairing-pairable-host._tcp`. Confirm the 6-digit code under Settings › Privacy & Security › Developer Mode › Pair with Host — no computer needed.

**iOS 18–25:** import an **RPPairing** file once from [idevice_pair](https://github.com/jkcoxson/idevice_pair/releases).
...
If you don't have a Mac, GitHub can build the app for you on one of its own
Mac runners and hand you back an **unsigned** `.ipa` — no Apple Developer
account, certificate, or provisioning profile required. This is meant for
sideloading tools (AltStore, SideStore, LiveContainer, etc.) that accept
unsigned/self-signed IPAs, or as a base for your own signing step. It is
**not** equivalent to an App Store– or TestFlight-signed build.

1. Open this repository on GitHub.
2. Click the **Actions** tab near the top of the page.
3. In the left sidebar, click **Build unsigned IPA**.
4. Click the **Run workflow** dropdown (top right of the list), optionally
   change **Build configuration** (defaults to `Release`), then click the
   green **Run workflow** button.
5. Wait for the run to finish (a few minutes) — it'll show a green check
   when done, or a red ✕ if something failed.
6. Click into the completed run, scroll down to **Artifacts**, and download
   **`LocusPlus-unsigned`**. It downloads as a `.zip` — unzip it once to get
   `LocusPlus-unsigned.ipa`.
7. Sideload `LocusPlus-unsigned.ipa` with your tool of choice.

If a run fails, open it and check the **build-logs** artifact (uploaded
automatically on failure) for `build.log`/`test.log`.
